...
Almost all or our R coding uses functions, which are essentially bits of code that have been written to do something.
 When we refer to functions we will generally do so following the convention `functionname()` the brackets are a giveaway that we are reffering to a function.
...
### Data types
R can deal with many different types of data and where it recognises differences in data it may treat it differently. 
...
When we use this type of data in R we have to remember the "" otherwise R will assume that we are refferring to objects (that may not have been created) of the same name and it likely to return an error
 
**Logical**

R understands logical data as TRUE FALSE, different from character data so no "" required and note that it is case sensative.
...
R is telling us that our vector is a character vector but we know that we have got some numeirc information in there.
...
We can extract information from a vector by *indexing*, using square brackets [] to specify the information that we want from the vector according to it's postition. 
...
We now have something that might look more familliar to those of us that are used to working with data in excel.
...
From this we can see that the data frame has 3 observations (rows) and 2 variables(columns), one calles animals which contains character data and one called weight_kg which contains numeric data. 
...
To etract a specific value we can *index* it using the square brackets [] to specify the position by location using dataframe[rownumber,columnnumber]
...
Brfore we go through importing your data to R it is worth taking some time to consider how to make this process as painfree as possible.
