...
This tutorial focusses more on the 'how', offering you some code chunks that you can modify (and reuse in future should you need to)to help you familiarise yourself with carrying out such operations in R, the 'why' pertaining to many of the concepts touched on here will be covered in more detail in the Summary Stats tutorial.

## Checking your data 

Once you have collected your data and read it in to R (if you need a recap of reading in data go to xyz... install turorial link...) the first thing to do is to check that you have the right data!
...
Some of the most useful functions from the `mosaic` package are listed [here](https://rdrr.io/cran/mosaic/man/aggregating.html) , a particularly usefule function you might want to checkout is `favstats()` that computes several statistics at once. 
...
One of the most powerful things that we can do with our data is to visualise it. Good data viz can help us to see what our data is doing and to communicate our findings to others. 
Several institutions including the BBC and the Financial times do the majority of their data visualisation using R. 

The below images are examples of the graphics that can be made using R
...
```{r}
knitr::include_graphics("images/cedric-birdcol.png")
```
...
If you are unsure of what plot would be best for your data you can consult the internet, sites such as [data-to-vis](https://www.data-to-viz.com/) offer some useful insight into how to decide the most appropriate dislay for your data 
...
gf_point(bill_depth_mm ~ bill_length_mm, colour = ~species, size = ~body_mass_g, data= penguins)

```

Now we have an informative plot that clearly shows the trends within each species.

You have just created a really sophisticated plot `r emo::ji("tada")`

The below code is simlar to the code you just used to create your plot but with a couple of extra details. 
Look at the code and see if you can work out what the extra bits are doing, run the code to check if you guessed correctly
...
The `alpha` argument alters the transparency of the points, this is usefuls when we have lots of points that are close together as it helps us to see where points are plotted on top of one another.
...
We use `scale_color_manual()` to create a custom colour pallete, here we have specified three colours by [name](https://www.r-graph-gallery.com/42-colors-names.html)

Add in a note about checking colour blind friendlyness of colours


`gf_theme()` is a function that allows us to alter the overall [theme](https://ggplot2.tidyverse.org/reference/ggtheme.html) of the plot, most noticably it has removed the grey background. 
...
#### Comparing numeric variables accross categories
...
task to modify the violin plot appearance - change colours  and theme
